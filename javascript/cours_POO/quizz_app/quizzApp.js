...
class Quiz {
    ...
    // prend en paramètre ce que l'utilisateur à cliqué
    guess(answer) {
        // si la réponse est bonne, on incrémente, sinon on ne fait rien
        if (this.getCurrentQuestion().isCorrectAnswer(answer)) {
            this.score++;
        }
        this.currentQuestionIndex++;
    }
    hasEnded() {    // si il retourne true, alors le questionnaire est fini 
...
quizApp = () => {
    if (quiz.hasEnded()) {
        ...
        // afficher les questions, les choix et la progression
        display.question();
        display.choices();
        display.progress();
    }
