...
// destructuration de movie
const Card = ({ movie }) => {
    // pour formater la date
    const dateFormater = (date) => {
        let [yy, mm, dd] = date.split('-'); // enlève les -
        //console.log([yy, mm, dd]);
        return [dd, mm, yy].join('/');
    };

    // pour trouver le genre du film
    const genreFinder = () => {
        let genreArray = [];
    ...
    const deleteStorage = () => {
        ...
        let newData = storedData.filter((id) => id != movie.id); // ne pas mettre !== sinon ça ne fonctionne pas, le film ne se supprime pas
